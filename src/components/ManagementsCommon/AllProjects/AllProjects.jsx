...
const AdminProjectsManager = () => {
    ...
    useEffect(() => {
        const fetchAllProjects = async () => {
            ...
            for (const statuss of statuses) {
                const colRef = collection(db, "stock", "projects", statuss);
    ...
    const filteredProjects =
        filter === "all"
            ? projects
            : projects.filter((p) => p.statuss === `${filter} projects`);
    ...
    const handleReject = async (project) => {
        try {
            ...
            await setDoc(rejectedRef, { ...project, statuss: "rejected projects", rejectedAt: new Date().toISOString() });
            ...
            setProjects((prev) =>
                prev.map((p) =>
                    p.id === project.id ? { ...project, statuss: "rejected projects" } : p
                )
            );
        } catch (error) {
                    ...
                    {filteredProjects.map((project) => (
                        ...
                        >
                            ...
                            <p className="text-sm text-gray-400 mb-4">
                                ...
                                <span
                                    className={
                                        project.statuss === "pending projects"
                                            ? "text-yellow-400"
                                            : project.statuss === "accepted projects"
                                                ? "text-green-400"
                                                : "text-red-400"
                                    }
                                >
                                    {project.statuss.replace(" projects", "").toUpperCase()}
                                </span>
                            </p>
